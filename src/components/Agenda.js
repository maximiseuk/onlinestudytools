...
export default () => {
    const
        ...
        undo = () => {
            setClientAgenda([...agenda.filter((x, i) => i < doneTodo[1]), doneTodo[0], ...agenda.filter((x, i) => i >= doneTodo[1])]);
            setDoneTodo([]);
        },
        createTodo = e => {
            e.preventDefault();
    ...
    useEffect(() => {
        ...
        /*fetch("/update_data", {
            ...
            credentials: "include",
            body: JSON.stringify(clientAgenda),
        })
        .then(res => res.json())
        .then(data => {
            if (data === "failed") {
        ...
        <>
                        ...
                        <Card className={classes.card}>
                            <CardContent>
                                <div style={{display: "flex", alignItems: "center"}}>
                                    <Typography variant="h5" gutterBottom style={{overflow: "hidden", textTruncate: "ellipsis",}} className={(new Date()).getTime() > new Date(todo.date).getTime() ? classes.late : null}>
                                        {todo.subject}
                                    </Typography>
                                ...
                                <Typography className="highlight">
                                    ...
                                    {todo.time === "All Day" ? todo.time : (new Date(todo.date)).toLocaleTimeString().split(":").slice(0, -1).join(":")}
                                    {(new Date()).getTime() > new Date(todo.date).getTime() && <span className={classes.late}> <LateIcon style={{marginBottom: -4,}} /> Late</span>}
                                </Typography>
            ...
            >
                <form onSubmit={changeDesc}>
                    <DialogTitle id="alert-dialog-title">Change todo description</DialogTitle>
                    <DialogContent>
                        <TextField
